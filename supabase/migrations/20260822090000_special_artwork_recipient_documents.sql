-- Special artwork transfers require recipient proof at 82%, regardless of amount.
-- The sender completes the transfer verification without a purpose-document gate.
  ...
  IF v_rec_user IS NOT NULL THEN
    ...
    IF v_special_artwork THEN
      v_required := jsonb_build_array(jsonb_build_object(
        ...
        'label', 'Preuve d''enregistrement des objets dans les serveurs internationaux ICOM/UNESCO'
      ));
      v_block_reason := 'Achat d''objets d''art spéciaux : preuve d''enregistrement ICOM/UNESCO requise côté destinataire.';
  ...
  IF v_rec_user IS NOT NULL THEN
    IF v_special_artwork THEN
      PERFORM public.notify_user(
        ...
        'Action requise — preuve ICOM/UNESCO demandée à 82%',
        'Le transfert pour achat d''objets d''art spéciaux est arrivé à 82%. Veuillez déposer la preuve d''enregistrement de vos objets dans les serveurs internationaux ICOM/UNESCO depuis votre tableau de bord. Aucun document supplémentaire n''est demandé à l''émetteur.',
